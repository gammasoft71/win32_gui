...
namespace {
  ...
  PIMAGE_THUNK_DATA FindAddressByOrdinal(void* moduleBase, PIMAGE_THUNK_DATA impName, PIMAGE_THUNK_DATA impAddr, WORD ordinal) {
    for (; impName->u1.Ordinal; ++impName, ++impAddr) {
  ...
  PIMAGE_THUNK_DATA FindDelayLoadThunkInModule(void* moduleBase, const char* dllName, WORD ordinal) {
    auto imports = DataDirectoryFromModuleBase<PIMAGE_DELAYLOAD_DESCRIPTOR>(moduleBase, IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT);
    for (; imports->DllNameRVA; ++imports) {
      if (_stricmp(RVA2VA<LPCSTR>(moduleBase, imports->DllNameRVA), dllName) != 0) continue;
      auto impName = RVA2VA<PIMAGE_THUNK_DATA>(moduleBase, imports->ImportNameTableRVA);
...
bool dark_mode::allow_dark_mode_for_window(HWND hWnd) {
  if (!g_darkModeEnabled) return false;
  if (g_darkModeSupported)
    return _AllowDarkModeForWindow(hWnd, g_darkModeEnabled);
  return false;
}
...
void dark_mode::refresh_title_bar_theme_color(HWND hWnd) {
...
void dark_mode::init_dark_mode(int enableDarkMode) {
