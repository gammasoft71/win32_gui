...
#include "../../../include/win32/forms/form.h"
...
void application::run() {
  while (do_events_());
  win32::forms::message message { 0, WM_QUIT, 0, 0 };
  wnd_proc(message);
}
...
bool application::do_events_() {
  ...
  if (PeekMessage(&message, nullptr, 0, 0, PM_NOREMOVE) != 0) {
    ...
    if (high_resolution_clock::now() - last_idle_time < 100ms) this_thread::sleep_for(2ms);
      ...
      win32::forms::message message{ 0, WM_APPIDLE, 0, 0 };
      wnd_proc(message);
...
void application::set_dark_mode() {
  ...
  if (!initialized) win32::forms::dark_mode::init_dark_mode(dark_mode_enabled());
  initialized = true;
}
